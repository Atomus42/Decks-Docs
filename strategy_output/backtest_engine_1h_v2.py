"""
Aggressive 1H backtesting engine — optimized for maximum returns.
Long-only, full equity commitment, realistic maker fees.
"""
import numpy as np
import pandas as pd
import json
...
FEE_RATE = 0.00075  # 0.075% maker fee (Binance/Coinbase Pro tier)
...
def run_backtest(df, signals, strategy_name, params_dict=None):
    """
    Long-only backtest with full equity commitment.
    ...
    for i in range(n):
        ...
        # Entry — use 99% of equity
        if prev_sig == 0 and sig == 1 and position == 0:
            position = (equity * 0.99) / price
            entry_price = price
