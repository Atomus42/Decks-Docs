"""
Shared backtesting engine for 1H multi-strategy analysis.
Adapted from the daily engine with higher fee assumptions for intraday trading.
"""
import numpy as np
import pandas as pd
import json
...
DATA_PATH = "/home/user/repo/strategy_output/btc_1h_candles.csv"
INITIAL_CAPITAL = 100_000.0
FEE_RATE = 0.0015  # 0.10% commission + 0.05% slippage per trade

def load_data():
    df = pd.read_csv(DATA_PATH, index_col=0, parse_dates=True)
    df.index.name = "Date"
    # Strip timezone if present for consistency
...
def run_backtest(df, signals, strategy_name, params_dict=None):
    """
    Run a vectorized long-only backtest on 1H data.

    Parameters:
        df: DataFrame with OHLCV data (1H bars)
    ...
    hourly_ret = eq_series.pct_change().dropna()
    ...
    # Sharpe annualized from hourly returns (sqrt(8760) for hours/year)
    ...
    avg_bars_per_trade = hours / n_trades if n_trades > 0 else 0
